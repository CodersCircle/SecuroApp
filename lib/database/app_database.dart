...
class AppDatabase extends _$AppDatabase {
  ...
  static QueryExecutor _openConnection() {
    return driftDatabase(name: 'securo_vault');
  }

  // ── Password CRUD ──────────────────────────────────────────
  Future<List<PasswordItem>> getAllPasswords() =>
      select(passwordItems).get();
  ...
  Future<List<PasswordItem>> searchPasswords(String query) =>
      (select(passwordItems)
        ..where((t) =>
  ...
  // ── TOTP CRUD ──────────────────────────────────────────────
  Stream<List<TotpAccount>> watchAllTotp() =>
      select(totpAccounts).watch();
